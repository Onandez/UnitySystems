...
namespace GameKit.InventorySystem
{
    ...
    /// </summary>
    public class InventoryDisplay : MonoBehaviour, EventListener<InventoryEvent>
    {
        [Header("Binding")]
        public string TargetInventoryName = "MainInventory";    // the name of the inventory to display
        ...
        public virtual void SetupInventoryDisplay()
        {
            ...
            if (this.gameObject.GetComponent<InventorySoundPlayer>() != null)
            {
                this.gameObject.GetComponent<InventorySoundPlayer>().SetupInventorySoundPlayer();
            }
        ...
        protected virtual void InitializeSlotPrefab()
        {
            ...
            _slotPrefab.GetComponent<Image>().raycastTarget = true;

            _slotPrefab.AddComponent<InventorySlot>();
            _slotPrefab.GetComponent<InventorySlot>().transition = Selectable.Transition.SpriteSwap;
            ...
            _slotPrefab.GetComponent<InventorySlot>().interactable = true;

            _slotPrefab.AddComponent<CanvasGroup>();
            _slotPrefab.GetComponent<CanvasGroup>().alpha = 1;
            _slotPrefab.GetComponent<CanvasGroup>().interactable = true;
            _slotPrefab.GetComponent<CanvasGroup>().blocksRaycasts = true;
            _slotPrefab.GetComponent<CanvasGroup>().ignoreParentGroups = false;
